...
from .metrics import FREE_CASH_FLOW_FORMULA, serialize_metric_schema
...
3. 只围绕四类指标组织内容：盈利指标、估值指标、现金流量指标、财务风险指标；忽略公司概况、股东结构等非目标信息。
4. 每只股票至少包含“核心结论”“分类指标分析”“风险与关注点”三个小节。
5. 尽量保留关键数值、同比/历史对比和股票代码。
6. 数据口径仅限近五年年度财报。
7. “自由现金流量”已经按公式预先计算，不要自行改写公式或替换为其他口径。
8. 如果某一部分数据缺失或查询失败，明确写“该部分数据不足”。
9. 不要输出免责声明，不要要求用户补充信息。"""
...
def build_fundamentals_formatting_prompt(
    ...
    return "\n".join(
        [
            ...
            "只允许围绕 metric_schema 中定义的基本面指标展开，严格使用四个分类：盈利指标、估值指标、现金流量指标、财务风险指标。",
            "只允许使用近五年年度财报数据，不要扩展到季度、半年报或其他时间范围。",
            f"现金流量指标中的自由现金流量已经按公式预先计算：{FREE_CASH_FLOW_FORMULA}。",
