...
- 基本面
    ...
    - 估值指标: 市盈率与市净率
    - 现金流量指标
        - 自由现金流量=净利润+固定资产和投资性房地产折旧+使用权资产折旧+无形资产摊销+长期待摊费用摊销-投资活动现金流出
        - 营业现金流量
    - 财务风险指标: 流动比率与资产负债率
- 技术面
    - MA
...
主流程通过 [src/fundamentals_agent/report_formatting.py](src/fundamentals_agent/report_formatting.py) 读取 `FUNDAMENTALS_LLM_PROVIDER`，再调用独立模块 [src/external_llm/clients.py](src/external_llm/clients.py) 中的通用 Chat Completions 客户端。如果未配置 provider、API Key 或模型名，报告生成会直接失败并返回明确错误。
...
仓库现在将通用外部 LLM 能力抽到独立包 [src/external_llm](src/external_llm) 下，方便后续新增的其他 agent 直接复用，而不需要再依赖 fundamentals agent 包内实现。fundamentals agent 自身只保留与“基本面报告整理”相关的 prompt 与序列化逻辑。

### 使用示例

```python
from external_llm import create_openai_client, create_zhipu_client
